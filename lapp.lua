...
local filetypes = {
    stdin = {process.stdin,'file-in'}, stdout = {process.stdout,'file-out'},
    stderr = {process.stderr,'file-out'}
}
...
local function error(msg,no_usage,...)
    quit(process.argv[0]:gsub('.+[\\/]','')..':'..msg,no_usage)
end
...
--p('OPEN',file,opt)
    local val,err = require('io').open(file,opt)
...
function process_options_string(str)
    ...
    for line in str:gfind('([^\n]*)\n') do
        local typespec,optspec,optparm,i1,i2,defval,vtype,constraint
        line = ltrim(line)
